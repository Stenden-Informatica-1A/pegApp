	...
	function sendJson(Query)
	{
		$.getJSON(Query, function(results)
		{
			...
			function getWeather()
			{
				...
				$(".dCond").html("Het weer van vandaag: " + results.forecast.forecastday[
					0].day.condition.text + ".<br><br><b>" + condition + "</b>");
				$(".weatherIcon").html("<img src='img/" + results.forecast.forecastday[
					0].day.condition.icon.substring(30) + "' alt='Weer icoon'>");
				if (eenheid == "metrisch")
				{
					$(".dAvgTemp").html(results.forecast.forecastday[0].day.avgtemp_c +
						" °C");
					$(".cWindSpeed").html(results.current.wind_kph + " km/u");
				}
				else if (eenheid == "imperiaal")
				{
					$(".dAvgTemp").html(results.forecast.forecastday[0].day.avgtemp_f +
						" °F");
				...
				$("#tDay").html(
					"<tr><th>Uur</th><th>Weer</th><th>Temp</th><th>Wind</th></tr>");
				for (i = sunRise; i < results.forecast.forecastday[0].hour.length; i++)
				{
					...
					//image
					tr.append("<td><img src='img/" + results.forecast.forecastday[0].hour[
							i].condition.icon.substring(30) + "' alt='" + results.current.condition
						.text + "'></td>");
					//temperature and windspeed
					if (eenheid == "metrisch")
					{
						tr.append("<td>" + results.forecast.forecastday[0].hour[i].temp_c +
							" °C</td>");
						tr.append("<td>" + results.forecast.forecastday[0].hour[i].wind_kph +
							" km/u</td>");
					}
					else if (eenheid == "imperiaal")
					{
						tr.append("<td>" + results.forecast.forecastday[0].hour[i].temp_f +
							" °F</td>");
						tr.append("<td>" + results.forecast.forecastday[0].hour[i].wind_mph +
							" mp/h</td>");
					}
					$('#tDay').append(tr);
				}
				...
				$("#tWeek").html(
					"<tr><th>Dag</th><th>Weer</th><th>Temp.</th><th>Luchtvocht.</th><th>Wind</th></tr>"
				);
				for (i = 0; i < results.forecast.forecastday.length; i++)
				{
					...
					tr.append("<td><img src='img/" + results.forecast.forecastday[i].day.condition
						.icon.substring(30) + "' alt='Weer icoon'><br>" + results.forecast.forecastday[
							i].day.condition.text + "</td>");
					//temp, humid, windspeed.
					if (eenheid == "metrisch")
					{
						tr.append("<td>" + results.forecast.forecastday[i].day.avgtemp_c +
							" °C</td>");
						tr.append("<td>" + results.forecast.forecastday[i].day.avghumidity +
							" %</td>");
						tr.append("<td>" + results.forecast.forecastday[i].day.maxwind_kph +
							" km/u</td>");
					}
					else if (eenheid == "imperiaal")
					{
						tr.append("<td>" + results.forecast.forecastday[i].day.avgtemp_f +
							" °F</td>");
						tr.append("<td>" + results.forecast.forecastday[i].day.avghumidity +
							" %</td>");
						tr.append("<td>" + results.forecast.forecastday[i].day.maxwind_mph +
							" mp/h</td>");
					}
					$('#tWeek').append(tr);
					day++;
				...
				if (max > 3)
				{
					$("#aIcon").html("<img src='img/" + iResult.substring(30) +
						"' alt='Weer icoon'>");
				}
				else
				{
					...
					$("#aIcon").html("<img src='img/day/" + wImg +
						".png' alt='Weer icoon'>");
				}
		...
		}).fail(function(d, textStatus, error)
		{
			...
			{
				melding =
					"De weerstation is niet berijkbaar, probeer het a.u.b. later opnieuw.";
			}
			if (error == "Bad Request")
			{
				error =
					"Ongeldige locatie, voer a.u.b. een andere locatie in. Zie settings"
			}
			$(".today").html("Error!");
			$("#week").html("Error!");
			$("#aText").html(d + "<br>" + melding + "<br>" + error + ".");
			$(".dCond").html("Er kunnen geen weer gegevens worden opgehaald! (" +
				textStatus + ".)<br><br>" + error + ".");
