...
function getPageSwipe()
{
	...
	if (localStorage.getItem('knijperName') && localStorage.getItem('knijperKey') &&
		localStorage.getItem('knijperKey') == "01")
	{
		...
		{
			// next page
			if (event.type == 'swipeleft' && page != "weatherWeek" && page !=
				"settings" && page != "firstLoad")
			{
	...
	{ //Terug sturen naar de pagina om je wasknijper toe te voegen.
...
function footerDots()
{
	...
	{
		$("#footer").remove();
		var pageDots = $.mobile.pageContainer.pagecontainer('getActivePage').attr(
			'id');
		if (pageDots == "main" || pageDots == "weatherDay" || pageDots ==
			"weatherWeek")
		{
			$("body").append(
				'<div id="footer"><div id="pageMarker"><div class="pageSelector"></div><div class="pageSelector"></div><div class="pageSelector"></div></div></div>'
			);
			if (pageDots == "main")
			{
...
$(document).on("backbutton", function(e)
{
	...
	if ($.mobile.activePage.attr("id") == "main" || $.mobile.activePage.attr(
			"id") == "firstLoad")
	{
...
function addKnijper()
{
	...
	alert("SMART-Knijper " + localStorage.getItem('knijperName') +
		" is toegevoegd!");
...
{
	alert(localStorage.getItem('knijperName') + ": " + localStorage.getItem(
		'knijperKey'));
}
...
{
	document.getElementById('notificaties').value = localStorage.getItem(
		'notificaties');
}
